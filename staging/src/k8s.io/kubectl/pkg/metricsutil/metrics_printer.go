...
func (printer *TopCmdPrinter) PrintNodeMetrics(metrics []metricsapi.NodeMetrics, availableResources map[string]v1.ResourceList, noHeaders bool, sortBy string) error {
	...
	for nodeName := range availableResources {
		printer.printMissingMetricsNodeLine(w, nodeName)
	}
	return nil
}
...
func (printer *TopCmdPrinter) PrintPodMetrics(metrics []metricsapi.PodMetrics, printContainers bool, withNamespace bool, noHeaders bool, sortBy string, sum bool) error {
	...
	for _, m := range metrics {
		if printContainers {
			...
			printer.printSinglePodContainerMetrics(w, &m, withNamespace)
		} else {
			printer.printSinglePodMetrics(w, &m, withNamespace)
		}
	...
	if sum {
		...
		printer.printPodResourcesSum(w, adder.total, columnWidth)
	}
...
func (printer *TopCmdPrinter) printSinglePodMetrics(out io.Writer, m *metricsapi.PodMetrics, withNamespace bool) {
	podMetrics := getPodMetrics(m, printer.measuredResources)
	if withNamespace {
...
func (printer *TopCmdPrinter) printSinglePodContainerMetrics(out io.Writer, m *metricsapi.PodMetrics, withNamespace bool) {
...
func (printer *TopCmdPrinter) printMetricsLine(out io.Writer, metrics *ResourceMetricsInfo) {
	printValue(out, metrics.Name)
	printer.printAllResourceUsages(out, metrics)
	fmt.Fprint(out, "\n")
}

func (printer *TopCmdPrinter) printMissingMetricsNodeLine(out io.Writer, nodeName string) {
	printValue(out, nodeName)
	unknownMetricsStatus := "<unknown>"
	for i := 0; i < len(printer.measuredResources); i++ {
...
func (printer *TopCmdPrinter) printAllResourceUsages(out io.Writer, metrics *ResourceMetricsInfo) {
	for _, res := range printer.measuredResources {
...
func (printer *TopCmdPrinter) printPodResourcesSum(out io.Writer, total v1.ResourceList, columnWidth int) {
