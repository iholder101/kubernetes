...
func (o TopNodeOptions) RunTopNode() error {
	...
	for _, n := range nodes {
		...
		if o.ShowSwap {
			summaryBytes, err := o.NodeClient.RESTClient().Get().
				AbsPath("/api/v1/nodes", n.Name, "proxy/stats/summary").
				DoRaw(context.TODO())
			if err != nil {
				return fmt.Errorf("failed to get node summary: %w", err)
			}
			...
			if summary.Node.Swap == nil || summary.Node.Swap.SwapAvailableBytes == nil || summary.Node.Swap.SwapUsageBytes == nil {
				return fmt.Errorf("failed to get swap memory information for node %s", n.Name)
			}
